...
events.on('order:submit', async () => {
    const orderDetailsInput = orderModel.getOrder();
    
    if (orderDetailsInput.email === '' || orderDetailsInput.phone === '' || orderDetailsInput.address === '' || !orderDetailsInput.items.length) {
    ...
    try {
        const response = await api.createOrder(orderDetailsInput);
...
// Повторное получение продуктов с сервера
api.getAllProducts()
    .then((res: ProductDetails[]) => {
        productModel.setProducts(res);
    })
